...
export function SourceSidebar({ sources, groups, libraryView, activeSourceId, libraryCounts, countsStale, collapsedGroups, onSelectLibrary, onSelectSource, onToggleGroup, onEditSource, onOpenSettings }: {
  ...
  const subscribedCount = sources.filter((source) => source.subscribed !== false).length;
  ...
  return <aside className="sidebar">
    <nav className="library-nav" aria-label="阅读分类">
      ...
      <LibraryFilter {...navigation} view="today" label="今日" icon="today" />
    ...
    <section className="source-section" aria-labelledby="source-heading">
      <div className="section-title" id="source-heading">来源 <span>{subscribedCount}</span></div>
      <div className="source-list">
        ...
        {!subscribedCount && <p className="empty-side">先添加一个公开 Feed 或网页。</p>}
      </div>
    </section>
...
export function Timeline({ loadingEntries, loadFailed, onReload, onAddSource, activeSource, libraryView, entrySearch, entries, hasMoreEntries, loadingMoreEntries, paginationError, sourceById, readingEntryId, notice, busy, onUndo, onEditSource, onClearNotice, onEntrySearchChange, onUpdateEntry, isEntryUpdating, onOpenEntry, onDismissEntry, onLoadMore }: {
  ...
  const title = activeSource?.title || ({ all: "全部内容", today: "今日", unread: "未读文章", favorite: "收藏文章" } satisfies Record<LibraryView, string>)[libraryView];
  ...
  return <section className="timeline" aria-label="文章列表">
    ...
    <div className="entry-list">
      {visibleEntries.map((entry) => <EntryCard key={entry.id} entry={entry} source={sourceById.get(entry.sourceId)} selected={readingEntryId === entry.id} onRead={onUpdateEntry} isEntryUpdating={isEntryUpdating} onOpen={onOpenEntry} onDismiss={onDismissEntry} busy={busy} />)}
...
function EntryCard({ entry, source, selected, onRead, isEntryUpdating, onOpen, onDismiss, busy }: { isEntryUpdating: EntryMutationPending; entry: Entry; source?: Source; selected: boolean; onRead: (entry: Entry, field: "read" | "favorite", value: boolean) => Promise<boolean>; onOpen: (entry: Entry) => void; onDismiss: (entry: Entry) => Promise<void>; busy: boolean }) {
  const date = entry.publishedAt
    ? new Intl.DateTimeFormat("zh-CN", { dateStyle: "medium" }).format(entry.publishedAt)
    : `收集于 ${new Intl.DateTimeFormat("zh-CN", { dateStyle: "medium" }).format(entry.createdAt)}`;
