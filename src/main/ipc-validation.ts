...
export function parseEntryListQuery(value: unknown): EntryListQuery | undefined {
  ...
  return {
    ...
    ...(value.collectedToday === undefined ? {} : { collectedToday: requireBoolean(value.collectedToday) }),
...
export function parseEntryPageQuery(value: unknown): EntryPageQuery | undefined {
  ...
  const parsedBase = parseEntryListQuery({
    ...
    collectedToday: value.collectedToday,
...
export function parseAiStreamRequest(value: unknown): AiStreamRequest {
  ...
  if (request.task !== undefined || request.translationTarget !== undefined || request.translationSegments !== undefined) {
    throw new Error("内置双语翻译已移除；请刷新应用后重试。");
  }
