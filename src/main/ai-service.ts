import type { RewriteSettings, RewriteRequestStage } from "../shared/rewrite";
...
export class AiService {
  ...
  async rewriteChunk(settings: RewriteSettings, prompt: string, signal: AbortSignal, stage: RewriteRequestStage = "write"): Promise<AiAnswer> {
  ...
  private async generate(providerId: AiProviderId, instruction: string, prompt: string, onDelta: AiDeltaListener, signal?: AbortSignal, selection?: RewriteSettings, rewriteStage?: RewriteRequestStage): Promise<AiAnswer> {
    throwIfAborted(signal, "AI 请求已取消。");
  ...
  private async askDeepSeekStream(endpoint: string, configuration: StoredAiConfiguration, prompt: string, instruction: string, onDelta: AiDeltaListener, signal?: AbortSignal, rewriteStage?: RewriteRequestStage): Promise<string> {
    const payload = {
      model: configuration.model,
      stream: true,
