...
// Font-face declarations register assets; only component rules must use tokens.
const componentStyles = css.replace(/:root\s*\{[^}]*\}/, "").replace(/@font-face\s*\{[^}]*\}/g, "");
...
if (!/\.article-body\s+\[data-reader-equation\]\s*>\s*\.reader-equation\s*\{[^}]*display:\s*grid[^}]*grid-template-columns:\s*minmax\(0,1fr\)\s+auto[^}]*width:\s*100%[^}]*min-width:\s*0/s.test(css)
  || !/\.article-body\s+\.reader-equation__viewport\s*\{[^}]*min-width:\s*0[^}]*overflow-x:\s*auto/s.test(css)
  || !/\.article-body\s+\.reader-equation__content\s*\{[^}]*width:\s*max-content[^}]*min-width:\s*100%/s.test(css)) {
  failures.push("公式容器必须由阅读器自有布局控制，并保持独立横向滚动能力");
}
if (!/\.article-body\s+\.reader-equation__tag\s*\{[^}]*align-self:\s*center[^}]*white-space:\s*nowrap/s.test(css)) {
