...
let db,snapshot,ai;try {
 ...
 const settings=db.rewrites.settings();if(!settings)throw Error("Configure a rewrite model first");
 ...
 ai=new AiService(new SecretStore());let calls=0;
 const sourceText=rewriteText(article);
 const result=await repairLegacyRewrite(sourceText,saved.result.sections || [saved.result.markdown],async(stage,prompt,signal)=>{
 ...
 await writeFile(report,JSON.stringify({entryId:entry.id,oldResult:saved.result,result:{...saved.result,markdown:result.markdown,sections:result.sections,structureRepair:{version:1,provider:settings.provider,model:settings.model,repairedAt:Date.now(),sourceHash:createHash("sha256").update(sourceText).digest("hex")}},settings,report:result.report}),{mode:0o600});
