...
async function auditViewport(window, viewport, mathJaxSvg) {
  ...
  const failures = [];
  const videoLayout = await window.webContents.executeJavaScript(`(() => { const v = document.querySelector('.reader-video video'); const block = v?.closest('.reader-video'); const body = v?.closest('.article-body'); if (!v || !block || !body) return false; const r=v.getBoundingClientRect(), b=body.getBoundingClientRect(); return r.width <= b.width + 1 && r.height <= innerHeight * .6 + 1 && block.scrollWidth <= block.clientWidth + 1; })()`);
  if (!videoLayout) failures.push("视频播放器或失败提示撑破正文列");
