...
function fixture() {
 ...
 const read=vi.fn(async()=>article);const rewriteChunk=vi.fn(async()=>({provider:"deepseek" as const,model:settings.model,text:"## 中文正文\n\n严谨改写的正文。"}));
...
it("queues immediately, deduplicates double clicks, and stores only derived content with provenance",async()=>{
 ...
 expect(f.db.rewrites.get("entry")?.result).toMatchObject({sourceUrl:f.entry.url,sourceTitle:"Original",model:settings.model,promptVersion:1});
...
it("preserves a completed draft on failed regeneration, with a snapshotted model",async()=>{
 ...
 expect(f.db.rewrites.get("entry")?.result).toEqual(result);expect(f.rewriteChunk.mock.calls[1][0].model).toBe(settings.model);
});
it("cancels a running job and rejects late output even after re-enqueuing",async()=>{
 ...
 await vi.waitFor(()=>expect(f.rewriteChunk).toHaveBeenCalledTimes(1));f.service.cancel("entry");expect(f.db.rewrites.get("entry")?.status).toBe("cancelled");
...
it("uses bounded complete chunks and does not expose remote errors",async()=>{
 ...
 expect(f.rewriteChunk.mock.calls.map(c=>c[1]).join("")).toContain("第 2/");
