...
function View({id}:{id:string}){const state=useArticleRewrite(id);return <><RewriteControls state={state} onSelect={state.setVisible}/><RewrittenArticle state={state}/></>;}
...
it("loads local output without generating, and preserves it on cancellation",async()=>{
 await act(async()=>root.render(<View id="one"/>));expect(generate).not.toHaveBeenCalled();await click("中文改写");expect(container.textContent).toContain("AI 中文改写");
 await click("重新生成改写");expect(container.textContent).toContain("已排队");expect(container.querySelector(".reader-rewritten")).not.toBeNull();await click("取消改写");expect(container.textContent).toContain("任务已取消");expect(container.querySelector(".reader-rewritten")).not.toBeNull();
});
...
it("lets users select the missing Chinese version without automatically generating",async()=>{
 get.mockResolvedValue(undefined);await act(async()=>root.render(<View id="one"/>));await click("中文改写");
 expect(container.textContent).toContain("尚未生成");expect(generate).not.toHaveBeenCalled();
 expect(container.querySelector('[aria-label="阅读版本"] button[aria-pressed="true"]')?.textContent).toBe("中文改写");
 await click("原文");expect(container.querySelector(".reader-rewritten")).toBeNull();
});
it("keeps Chinese readable with review issues and failed background checks",async()=>{
 ...
 await act(async()=>root.render(<View id="one"/>));await click("中文改写");
 expect(container.textContent).toContain("中文仍可正常阅读");expect(container.textContent).toContain("检查未完成");expect(container.textContent).toContain("第 16 节");expect(container.querySelector(".article-body")).not.toBeNull();
