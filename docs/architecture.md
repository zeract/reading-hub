...
阅读导航为今日、全部内容、未读和收藏，默认今日。今日查询在本地自然日内对 `published_at` 与首次写入的 `created_at` 取并集，列表、分页和计数复用同一 SQL 条件；重复同步不改变收集日。排序继续采用已有的发布时间优先稳定游标。
...
删除和恢复同样分开处理写入与后续读取：删除提交后保留撤销通知，读取错误不能覆盖其身份和恢复对象；真正的删除失败不关闭阅读器，也不生成撤销。卡片只提供删除操作；通知保留即时撤销，不再提供最近删除视图。删除标记仍用于阻止同步把已删除内容重新带回。

取消订阅与 OPML 导入同样以主进程命令结果判断成功。列表重读失败仍由读模型提示并恢复，不让已完成的订阅命令停留在失败弹窗，也不丢失已返回的导入数量。命令本身失败继续向所属弹窗传递错误；取消文件选择不触发列表重读。

工具栏重载和来源刷新在开始时向 useLibraryNotice 取得一次性完成发布函数，期间保留现有通知。新的刷新、直接通知、有效的通知更新或显式关闭会同步使旧发布函数失效；刷新成功与失败共用该归属检查，不能用过期完成结果替换后来产生的删除撤销。列表过期保护和弹窗自身的错误反馈仍分别由原来的所有者负责。

来源刷新与随后列表读取分别判断结果。来源操作成功后，列表失败只留下读模型错误，不进入来源失败分支重复读取，也不让已保存收集范围再次等待来源刷新。真正的来源失败仍尝试一次列表读取以呈现最新健康状态，并将原始来源错误传给弹窗；列表读取失败不能替换该错误。

已读和收藏在命令被接受时捕获错误发布函数，普通启动不占用下一条通知的优先级；实际失败只在捕获的通知状态仍有效时显示。合并的重复意图共享发布者，排队的相反意图各自保留入队时的归属，不能在稍后开始 IPC 时认领已经更新的通知。写入成功、失败返回值和重试可用性不受通知是否过期影响。
...
收集范围编辑器将当前发现的分类、已保存选择和本次编辑过的分类按共享 facetIdentity 合并为可见选项。缺少当前目录元数据的分类仍可取消或重新选择，不补造文章计数；可见选项不是保存范围，只有当前勾选的身份写入。分类发现沿用显式能力标志，范围变化使用与主进程相同的 sameSubscriptionScope 判断。历史回填已移除，迁移 11 将旧回填范围归零；RSS 只获取当前 Feed，旧卡片与分类选择保留。
...
校准检测与规则保存通过 SourceService 的同一资格检查，复用 sourceCapabilities.canCalibrate；只有仍处于订阅状态的普通网页来源可以进入。异步检测完成后重新检查资格，规则保存也在任何重置和同步取消之前检查，界面状态不代替主进程验证。
...
添加来源将已选方式、键盘焦点和选择区展开状态分开管理。方向键只移动焦点，原生 Enter/Space 点击才激活方式；离开标签列表后恢复已选项的 Tab 入口。标签和面板由同一静态方式表及 useId 建立关联；隐藏面板保留语义容器但不挂载连接表单，实际切换仍触发既有视图清理，浏览焦点则不影响草稿或当前探测。

搜索与划词工具栏共享 Escape 事件契约：组合输入中的按键归输入法，已被内部控件处理的按键不再关闭外层工作流。搜索清除和划词关闭执行时标记 preventDefault，让同一次按键只完成对应操作；原生模态框继续拥有自己的取消和焦点恢复行为。

分类目录发现按请求时的来源配置归属结果：来源地址、类型、连接器、账号、连接器配置和提取规则参与比较，展示字段、轮询与用户收集范围不参与。远端成功或失败后均先处理取消，再验证来源仍存在且配置等价；成功结果随后与最新本地分类及数量合并。比较使用深度结构相等，不因配置对象键顺序不同而误判；整个发现过程不写文章或范围。
...
SourceService.delete 复用数据库的来源关系删除事务，移除信源独有文章（包括收藏）、相关来源记录与无内容引用的删除标记；共享文章先重新指定剩余来源，保留收藏及已读状态。命令可安全重试，提交后取消旧同步，迟到结果不能写回已删除来源。旧版 subscribe=false IPC 也路由到同一删除流程。

知乎关注来源先停用同步并清理独立登录会话，再删除数据库记录；清理失败保留停用记录供重试。应用服务初始化在任何调度启动前处理旧的取消订阅来源；失败关闭已组装的服务，下一次启动继续处理。暂停来源不属于清理范围。
