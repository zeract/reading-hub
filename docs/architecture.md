...
收集范围编辑器将当前发现的分类、已保存选择和本次编辑过的分类按共享 facetIdentity 合并为可见选项。缺少当前目录元数据的分类仍可取消或重新选择，不补造文章计数；可见选项不是保存范围，只有当前勾选的身份写入。分类发现与历史导入分别使用各自能力标志，范围变化使用与主进程相同的 sameSubscriptionScope 判断。

来源配置的收集范围读取使用独立的 useAsyncAction 生命周期，读取与保存/刷新错误各自拥有状态。首次读取和重试都拒绝旧会话结果，读取恢复不覆盖名称或文件夹草稿，也不写入范围；范围未载入时只允许既有基本配置路径保存，不能把未知范围当成空范围提交。

自动校准按规则提交、来源刷新、列表完成回调三个阶段推进。规则提交会重置来源采集结果，因此提交成功后锁定候选与重新检测，恢复动作只能继续未完成阶段；视图状态通过 useAsyncAction 的当前会话检查发布，关闭后已接受的写入及其完成回调仍沿既有流程收尾。该阶段状态只属于当前弹窗，重启后的采集恢复仍由持久化规则、失效的 HTTP 校验器和既有调度负责。

添加来源的五种方式共用 SourceActionFeedback 输出操作状态与纯文本错误，复用既有 useAsyncAction 的互斥和过期响应保护。学术候选的 undefined 表示尚无当前查询结果，空数组表示已完成且无匹配；编辑或开始新查询立即清除旧结果。显示进度根据实际操作区分搜索/订阅与探测/OPML 导入，不从单一 busy 值猜测操作类型。
