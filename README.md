...
在“设置 → AI 功能 → 中文改写”单独保存改写服务与模型，再打开文章点击“生成中文改写”。使用已有的 Keychain 凭据，仅主动生成时向所选服务发送从文章原始链接读取的正文。不会随订阅自动生成或改变 AI 学习的模型。

任务在应用运行期间后台排队，切换文章不影响生成；可取消、重新生成、删除改写，并在原文与本地改写之间切换。排队任务重启后继续，退出或崩溃中断的运行任务可手动重试。长文按段落、代码和公式分段，超过限制或只有摘要时明确失败，不截取前半篇冒充完整改写。

生成前先统一全文提纲与术语，各节携带相邻上下文改写，再逐段对照原文检查。发现问题时只修订对应节并复查；提纲或审阅格式异常时会指出字段问题并自动重试该步骤一次；仍异常或修订后仍有问题就停止，不替换旧稿。界面显示处理阶段，保存结果附带检查节数/段数；模型检查仍可能漏判，不能保证事实准确性。这个流程会增加模型调用次数和等待时间。已有改写不会自动重新生成。
