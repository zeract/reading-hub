...
收藏工具栏左侧以无边框下拉框提供“原文 / 中文改写”切换，选择尚未生成的版本不会调用模型。默认每节一次改写，短文一次完成，长文携带已生成上下文保持术语和衔接；不再强制逐节提纲、审阅、修订。原文中以普通字符书写的数学表达式也由模型输出为带分隔符的 TeX，保留上下标分组；代码不转换。完整中文立即保存。失败后重试会复用原文、模型及提示版本一致的已完成分段，成功后清除恢复记录。

完成后只显示标题和正文，不显示额外说明、生成元数据或重新生成/删除/检查按钮。中文正文使用朱雀仿宋，英文保留 Georgia；图片经现有主进程代理显示，链接使用标准 Markdown 解析，裸露网址收起为简短链接文字。版本菜单仅在当前版本下方展开另一个版本，不重复显示当前选项，背景与阅读器一致；原文和中文改写都支持 AI 对话与划词操作，对话使用当前版本内容。旧稿中的有效图片 Markdown 可直接显示，不会自动调用模型重新生成。
