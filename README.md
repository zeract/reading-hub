...
收藏工具栏左侧以两个无边框文字标签提供“原文 / 中文改写”切换，选择尚未生成的版本不会调用模型。默认每节一次改写，短文一次完成，长文携带已生成上下文保持术语和衔接；不再强制逐节提纲、审阅、修订。原文中以普通字符书写的数学表达式也由模型输出为带分隔符的 TeX，保留上下标分组；代码不转换。公式、编号、引用、代码和图片使用稳定标记保护，生成后本地恢复；链接保留原始目标，只翻译对应锚文本。完整中文立即保存。失败后重试会复用原文、模型及提示版本一致的已完成分段，成功后清除恢复记录。
