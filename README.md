...
任务在应用运行期间后台排队，切换文章不影响生成；生成中可取消，并在原文与本地改写之间切换。排队任务重启后继续，退出或崩溃中断的运行任务可手动重试。长文按完整的 Markdown 内容块分段，保留标题层级、列表嵌套、表格、图注、代码和公式，超过限制或只有摘要时明确失败，不截取前半篇冒充完整改写。

收藏工具栏左侧以两个无边框文字标签提供“原文 / 中文改写”切换，选择尚未生成的版本不会调用模型。默认每节一次改写，短文一次完成，长文携带已生成上下文保持术语和衔接；不再强制逐节提纲、审阅、修订。原文中以普通字符书写的数学表达式也由模型输出为带分隔符的 TeX，保留上下标分组；代码不转换。推荐卡片先按原文局部结构整理为一个标题链接；公式、编号、引用、代码和图片使用稳定标记绑定所属正文块，生成后本地恢复并检查标题、列表和表格结构；链接保留原始目标，只翻译对应锚文本。完整中文立即保存。失败后重试会复用原文、模型及提示版本一致的已完成分段，成功后清除恢复记录。

完成后只显示标题和正文，不显示额外说明、生成元数据或重新生成/删除/检查按钮。中文正文使用朱雀仿宋，英文保留 Georgia；图片经现有主进程代理显示，链接使用标准 Markdown 解析，原始目标由程序保存，模型翻译锚文本后恢复文字链接。当前版本以下划线标识，切换不弹出菜单或遮挡正文；原文和中文改写都支持 AI 对话与划词操作，对话使用当前版本内容。旧稿中的有效图片 Markdown 可直接显示，不会自动调用模型重新生成。
...
旧稿结构修复工具：构建后设置 `READING_HUB_REPAIR_ENTRY` 与 `READING_HUB_REPAIR_REPORT`，运行 `electron scripts/repair-rewrite-app`。工具在数据库快照中对照原文恢复链接、图片及可唯一匹配的公式编号，按已配置模型翻译和匹配锚文本；需要用户授权发送旧稿相关段落。它只输出本地修复报告，不写入在线数据库，替换前必须校验、备份并比较旧结果，拒绝覆盖正在生成或已变化的稿件。

维护工具增加 `READING_HUB_REPAIR_CARDS_ONLY=1`：仅对照原文修复旧稿推荐卡片，不调用模型。沿用快照、报告和写回前备份/并发比较要求。渲染与分段共用 Markdown 解析器，原站及模型 HTML 不执行，图片仍通过主进程代理加载。
