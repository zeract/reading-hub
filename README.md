...
顶部固定提供“原文 / 中文改写”切换，选择尚未生成的版本不会调用模型。默认每节一次改写，短文一次完成，长文携带已生成上下文保持术语和衔接；不再强制逐节提纲、审阅、修订。完整中文立即保存。失败后重试会复用原文、模型及提示版本一致的已完成分段，成功后清除恢复记录。

“检查改写”是可选的额外模型调用，只对保存的中文附加待核对项；衔接建议或检查失败不影响阅读，不自动修改正文。原文发生变化时要求重新生成后再检查。旧版改写仍可阅读，新生成版本支持分节检查。模型检查不能保证事实准确性。
