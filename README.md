...
完成后只显示标题和正文，不显示额外说明、生成元数据或重新生成/删除/检查按钮。中文正文使用朱雀仿宋，英文保留 Georgia；图片经现有主进程代理显示，链接使用标准 Markdown 解析，裸露网址收起为简短链接文字。版本菜单在当前版本文字下方展开，背景与阅读器一致；原文和中文改写都支持 AI 对话与划词操作，对话使用当前版本内容。旧稿中的有效图片 Markdown 可直接显示，不会自动调用模型重新生成。
