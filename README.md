...
生成前先统一全文提纲与术语，各节携带相邻上下文改写，再逐段对照原文检查。无法确认的辅助术语候选会被忽略，不会阻断整篇；长文提纲会按上下文预算节选并明确标记，每节完整原文仍参与生成和检查；正文覆盖与事实引文仍严格校验。发现问题时只修订对应节并复查；提纲或审阅格式异常时会指出字段问题并自动重试该步骤一次；仍异常或修订后仍有问题就停止，不替换旧稿。界面显示处理阶段，保存结果附带检查节数/段数；模型检查仍可能漏判，不能保证事实准确性。这个流程会增加模型调用次数和等待时间。已有改写不会自动重新生成。
